...
public class TeamDetailAdapter extends BaseRecyclerViewAdapter<ModelCardViewHolder, TeamDetailAdapter.UserAdapterListener> {
    ...
    public void onBindViewHolder(ModelCardViewHolder baseTeamViewHolder, int position) {
        ...
        if (item instanceof Ad) ((ContentAdViewHolder) baseTeamViewHolder).bind((Ad) item);
        else if (item instanceof Role) ((RoleViewHolder) baseTeamViewHolder).bind((Role) item);
        else ((JoinRequestViewHolder) baseTeamViewHolder).bind((JoinRequest) item);
    }
