...
public class MediaRepository extends ModelRespository<Media> {
    ...
    public Single<Media> createOrUpdate(Media model) {
        MultipartBody.Part body = ModelRespository.getBody(model.getImageUrl(), Media.UPLOAD_KEY);
        ...
        return mediaSingle.map(localMapper(model)).map(getSaveFunction()).observeOn(mainThread());
    }
